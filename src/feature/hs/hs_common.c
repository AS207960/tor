...
/** Parses a CAA line and returns a new hs_caa_config_t,
 * or NULL and an optional error string on failure.
 *
...
hs_parse_caa_config(const char *string, char **err_msg_out)
{
  ...
  smartlist_split_string(sl, string, " ",
                         SPLIT_SKIP_SPACE|SPLIT_IGNORE_BLANK|SPLIT_STRIP_SPACE, 3);
  if (smartlist_len(sl) < 3 || BUG(smartlist_len(sl) > 3)) {
    err_msg = tor_strdup("Bad syntax in hidden service CAA line.");
    goto err;
  }
  int ok;
  ...
  const char *l_tag = smartlist_get(sl,1);
...
/** Release all storage held in a hs_caa_config_t. */
void
hs_caa_config_free_(hs_caa_config_t *p)
{
  tor_free(p->tag);
  tor_free(p->value);
  tor_free(p);
